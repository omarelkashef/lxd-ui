...
import { useImageRegistries } from "context/useImages";
...
import type { LxdImageRegistry, LxdImageRegistryProtocol } from "types/image";
import usePanelParams, { panels } from "util/usePanelParams";
import ImageRegistryDetailPanel from "./ImageRegistryDetailPanel";

const isRegistryPublic = (registry: LxdImageRegistry): boolean => {
  return registry.config?.public?.toLowerCase() === "true";
};

export const ImageRegistriesList: FC = () => {
  ...
  const deletableImageRegistries = filteredImageRegistries.filter(
  ...
  const rows = filteredImageRegistries.map((registry) => {
    ...
    const openSummary = () => {
      panelParams.openImageRegistry(registry.name);
    };

    const hasActivePanel = panelParams.imageRegistry === registry.name;
    return {
      key: registry.name,
      name: registry.name,
  ...
  return (
    <>
        ...
        header={
          <PageHeader>
            <PageHeader.Left>
              ...
              {selectedImageRegistries.length > 0 && (
                <Button
                  id="bulk-delete-image-registries-btn"
                  name="Delete"
                  disabled={deletableImageRegistries.length === 0}
                  hasIcon
                  className="u-float-right u-no-margin--bottom"
                  title="Delete image registries"
                >
                  <Icon name="delete" className="u-margin--right" />
                  <span>Delete</span>
                </Button>
              )}
            </PageHeader.Left>
      ...
      {panelParams.panel === panels.imageRegistry && (
        <ImageRegistryDetailPanel />
      )}
    </>
  );
};
